...
app.use(session({
  ...
  store: new OrientoStore({server: "host=localhost&port=2424&username=root&password=wjs3250@@&db=jeon&useToken=true"}),
}));
...
passport.use(new FacebookStrategy({
  clientID: '370487720723589',
  clientSecret: 'dd85799205713dbe399414f19f5926cb',
...
app.post('/auth/login', 
  passport.authenticate('local', { successRedirect: '/welcome', failureRedirect: '/auth/login', failureFlash: false }))

app.get('/auth/facebook', passport.authenticate('facebook', {scope:'email'}));
app.get('/auth/facebook/callback',
  passport.authenticate('facebook', { successRedirect: '/welcom',
...
app.post('/auth/register', function(req, res){
  hasher({password:req.body.password}, function(err, pass, salt, hash){
    ...
    db.query(sql, {params:{user}}).then(function(results){
